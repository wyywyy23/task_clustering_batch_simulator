...
#include <Simulator.h>
#include "ZhangFixedGlobalWMS.h"
#include <Util/ProxyWMS.h>
#include <Util/PlaceHolderJob.h>
#include <assert.h>

XBT_LOG_NEW_DEFAULT_CATEGORY(zhang_fixed_global_wms,
                             "Log category for Zhang Fixed Global WMS");
...
namespace wrench {
    ...
    void ZhangFixedGlobalWMS::applyGroupingHeuristic() {
...
//      WRENCH_INFO("APPLYING GROUPING HEURISTIC");
        ...
        if (not individual_mode) {
            ...
            std::cout << "Submitting runtime: " << partial_dag_makespan << ", nodes: " << parallelism << std::endl;
    ...
    void ZhangFixedGlobalWMS::processEventPilotJobStart(std::shared_ptr<PilotJobStartedEvent> e) {
...
//                  this->pending_placeholder_job->pilot_job->getName().c_str());
    ...
    void ZhangFixedGlobalWMS::processEventPilotJobExpiration(std::shared_ptr<PilotJobExpiredEvent> e) {
...
//      double wasted_node_seconds = e->pilot_job->getNumHosts() * e->pilot_job->getDuration();
    ...
    void ZhangFixedGlobalWMS::processEventStandardJobCompletion(std::shared_ptr<StandardJobCompletedEvent> e) {
...
//                    this->simulator->used_node_seconds += t->getFlops() / this->core_speed;
    ...
    void ZhangFixedGlobalWMS::processEventStandardJobFailure(std::shared_ptr<StandardJobFailedEvent> e) {
    ...
    unsigned long ZhangFixedGlobalWMS::getStartLevel() {
    ...
    ZhangFixedGlobalWMS::groupLevels(unsigned long start_level, unsigned long end_level) {
